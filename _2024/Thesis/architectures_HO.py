...
class NetworkArchitectureHarmonicOscillator(Scene):
    def construct(self):
        ...
        arrows.add(
            Arrow(
                start=encoder_component[0][-1][0].get_corner(UR),
                end=koopman_component[0][0][0].get_corner(DL),
                buff=0.1,
                stroke_width=2
            )
        )
        arrows.add(
            Arrow(
                start=koopman_component[0][-1][0].get_corner(DR), 
                end=decoder_component[0][0][0].get_corner(UL), 
                buff=0.1, 
                stroke_width=2
            )
        )
        ...
        self.play(self.camera.frame.animate.scale(1.6))
