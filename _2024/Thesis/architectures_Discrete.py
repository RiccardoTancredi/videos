...
class NetworkArchitectureDiscrete(Scene):
    def construct(self):
        # --- Funzione helper per creare componenti ---
        def create_component(shape, inside_str, label_str, shift_type=None):
            # Testo interno con specifiche (numero di layer, neuroni, ecc.)
            inside_text = Tex(inside_str, font_size=30)
        ...
        def create_connected_component(num_layers, inside_strs, label_str, brace_pos=DOWN, text_color=WHITE,
            ...
            brace_position = 'underbrace' if (brace_pos == DOWN).all() else 'overbrace'
            symb = '_' if (brace_pos == DOWN).all() else '^'
            brace = Tex(R"\%s{\hspace{%0.2fcm}}%s{\mathrm{%s}}" % (brace_position, total_width, symb, label_str), font_size=40)
            
            # Posizionare l'underbrace sotto il gruppo di rettangoli
            brace.next_to(shapes, brace_pos, buff=0.2)

            return VGroup(shapes, brace)
        ...
        koopman_component = create_connected_component(
            num_layers=4, 
            inside_strs=["10", "10", "10", "2"], 
            label_str="Koopman", 
            fill_color=get_opposite_color("#6290C8"),
            brace_pos=UP
        )
        ...
        decoder_component = create_connected_component(
            num_layers=4, 
            inside_strs=["30", "30", "30", "2"],
            label_str="Decoder",
            fill_color=get_opposite_color("#829CBC"),
            brace_pos=DOWN
        )
        ...
        middle_position = (encoder_component.get_center() + decoder_component.get_center()) / 2
        koopman_component.move_to(middle_position + 2.5*UP)
        # koopman_component.next_to(encoder_component, DOWN, buff=1.5)
        ...
        # Simple Arrows
        # arrows.add(Arrow(encoder_component[0][-1][0].get_bottom(), koopman_component[0][0][0].get_top(), 
        #     buff=0.1, stroke_width=2))
        # arrows.add(Arrow(koopman_component[0][-1][0].get_top(), decoder_component[0][0][0].get_bottom(), 
        #     buff=0.1, stroke_width=2))

        # Curved Arrows
        arrows.add(
            Arrow(
                start=encoder_component[0][-1][0].get_top(),# corner(UR),
                end=koopman_component[0][0][0].get_bottom(),#corner(DL),
                buff=0.1,
                stroke_width=2
            )
        )
        arrows.add(
            Arrow(
                start=koopman_component[0][-1][0].get_bottom(),#corner(DR), 
                end=decoder_component[0][0][0].get_top(),#corner(UL), 
                buff=0.1, 
                stroke_width=2
            )
        )
        ...
        self.add(components, koopman_component, arrows)
        self.wait(5)
